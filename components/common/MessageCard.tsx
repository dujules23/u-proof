...
const MessageCard: FC<Props> = ({
  ...
  return (
    <Link
      href={`/messages/${_id}`}
      className="rounded-lg shadow-md p-4 shadow-secondary-dark dark:shadow-grey-100 overflow-hidden bg-white dark:bg-gray-800 transition flex flex-col h-full space-y-3"
    >
      <div>
      ...
      <div>
        <div className="dark:text-primary-light text-primary-dark">
          <FormattedMessageDisplay content={message} maxLength={30} isPreview />
        </div>
      </div>

      <div className="text-primary-light mt-8 p-2 flex-1 flex flex-col">
        {approved ? (
